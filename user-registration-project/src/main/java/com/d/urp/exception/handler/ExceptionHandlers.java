...
public class ExceptionHandlers {
	...
	public ResponseEntity<ErrorResponse> handler(RuntimeException run) {
		...
		return ResponseEntity.status(HttpStatus.BAD_REQUEST)
				.body(ErrorResponse.builder().status("error").code(run.getMessage()).massage(errorMassage.get(run.getMessage())).build());
	}
